...
<div class="container py-3">
    ...
    <div class="mt-2 tab-day d-flex gap-2 flex-wrap">
        <?php if (empty($days)): ?>
            <div class="text-muted">15日以上先の日付がありません。</div>
        <?php else: ?>
    ...
    <?php if (!$canGroup): ?>
        <div class="alert alert-warning mt-3">この画面は集団予約権限が必要です。</div>
    <?php endif; ?>
    ...
    <div class="excel-card mt-3 p-3">
        <form action="<?= $this->Url->build(['action' => 'bulkAddSubmit']) ?>" method="post" id="reservation-form">
            ...
            <input type="hidden" name="reservation_type" value="group">
            ...
            <div class="table-responsive">
                <table class="table excel-table align-middle">
                    ...
                    <tbody id="user-rows">
                    <tr><td colspan="7" class="text-center text-muted">部屋を選択してください。</td></tr>
                    </tbody>
                </table>
            </div>
        </form>
...
<?= $this->Html->script('bulk_add_form.js') ?>
</body>
</html>
