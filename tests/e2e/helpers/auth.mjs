...
/**
 * 直前編集ページを開き「個人（部屋別）」タブへ切り替える。
 *
 * 送信ハンドラのバインド完了を待ってから切り替える。バインド前に送信すると
 * ネイティブ送信へ落ちるため、待たないとテストが実態を見誤る。
 *
 * @param {import('@playwright/test').Page} page
 * @param {{room: number, date: string, meal: number}} target
 */
export async function openIndividualTab(page, { room, date, meal }) {
    await page.goto(appPath(`/TReservationInfo/changeEdit/${room}/${date}/${meal}`));
    await page.waitForFunction(
        () => document.getElementById('change-edit-form')?.dataset.submitBound === '1',
        null,
        { timeout: 20000 }
    );
    await page.click('.ce-type-btn[data-ce-type="1"]');
    await page.waitForTimeout(200);
}

/** 個人タブの食事チェックボックス（同名 hidden と区別するため type を明示）。 */
export function mealCheckbox(page, { meal, room }) {
    return page.locator(
        `#ce-room-tbody input.meal-checkbox[type="checkbox"][name="meals[${meal}][${room}]"]`
    );
}

/** ダイアログを閉じつつ内容を記録する。 */
export async function captureDialogs(page) {
    const seen = [];
